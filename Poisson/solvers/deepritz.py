...
def DeepRitz_solver(
    ...
    os.makedirs("DeepRitz_experiments", exist_ok=True)
    save_path = os.path.join(save_path, "DeepRitz_experiments")
    ...
    for t in range(iter):
        t_0 = time.time()
        net_u.zero_grad()
        loss_Ritz = losses.Deep_Ritz_loss(net_u, N_r, N_b, bd_lambda)
        loss_Ritz.backward(retain_graph=True)
        optim_u.step()
        t_1 = time.time()
        ...
        if (t + 1) % plot_period == 0:
            ...
            plotting_utils.Two_D_plot_heatmap_error_cmap(
                0.5, net_u, L, N_plot, chosen_dim_0, chosen_dim_1, t + 1, 1, save_path, 0, 0.1, device
            )
    ...
    filename = os.path.join(save_path, "netu_DeepRitz.pt")
