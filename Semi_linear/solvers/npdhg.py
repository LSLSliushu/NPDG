...
def PDHG_solver_extrapolation_in_func_space_Bdry_loss(
    ...
    net_phi = models.network_dual(network_length, config.dim, hidden_dimension_net_phi, 1, R).to(device)
    ...
    for t in range(iter):
        ...
        for _ in range(phi_psi_iter):
            lossa = losses.PDHG_loss(net_u, net_phi, net_psi, in_samples, bd_samples, bd_lambda) - (
            ...
            net_phi_auxil = models.network_dual(
                network_length, config.dim, hidden_dimension_net_phi, 1, R
            ).to(device)
            ...
            print("phi minres info")
            print(info_phi)

            lossa2 = losses.PDHG_loss(net_u, net_phi, net_psi, in_samples, bd_samples, bd_lambda) - (
        ...
        net_phi_0 = models.network_dual(network_length, config.dim, hidden_dimension_net_phi, 1, R).to(device)
        ...
        if (t + 1) % plot_period == 0:
            ...
            plotting_utils.Two_D_plot_graph_nn_primal_with_real_solution(
                net_u, R, N_plot, t, flag_plot_real, save_path, z_min, z_max, device
            )
