...
def PDHG_solver_extrapolation_in_func_space_Bdry_loss(
    ...
    if precond_type == "MpMd_Id":
        ...
        G_psi_type = "4"
    elif precond_type == "MpMd_nabla":  # another tested preconditioner in the paper
        G_u_type = "24"
        ...
        loss_psi = losses.L2_norm_sq_psi
        G_psi_type = "4"
    elif precond_type == "MpMd_nabla_H1":
        G_u_type = "25"
        loss_phi = losses.L2_norm_sq_nabla_phi
        G_phi_type = "2"
        loss_psi = losses.X_bdd_norm_psi
        G_psi_type = "5"
    elif precond_type == "MpMd_sqrt_kappa_nabla_H1": # we use this preconditioner in the current implementation
        G_u_type = "2b5"
        loss_phi = losses.L2_norm_sq_sqrt_A_nabla_phi
        G_phi_type = "2b"
        loss_psi = losses.X_bdd_norm_psi
        G_psi_type = "5"
    else:
    ...
    for t in range(iter):
        ...
        bd_samples, bdd_idx = sampling.rho_bdry_sampler_X_bdd_norm(N_b)
        ...
        for _ in range(phi_psi_iter):
            lossa = losses.PDHG_loss_X_bdd_norm(
                net_u, net_phi, net_psi, in_samples, bd_samples, bdd_idx, bd_lambda
            ...
            G_inv_nabla_eta_loss, info_phi = minres_utils.minres_solver_G(
                net_phi,
                net_phi_auxil,
                in_samples,
                bd_samples,
            ...
            lossa2 = losses.PDHG_loss_X_bdd_norm(
                net_u, net_phi, net_psi, in_samples, bd_samples, bdd_idx, bd_lambda
            ) - epsilon / 2 * bd_lambda * loss_psi(net_psi, bd_samples, bdd_idx)
            ...
            G_inv_nabla_eta2_loss, info_psi = minres_utils.minres_solver_G(
                net_psi,
                net_psi_auxil,
                in_samples,
                bd_samples,
        ...
        net_phi_0 = models.network_dual(network_length, config.dim, hidden_dimension_net_phi, 1, L).to(device)
        torch.nn.utils.vector_to_parameters(original_eta, net_phi_0.parameters())
        net_psi_0 = models.network_dual_on_bdry(network_length, config.dim, hidden_dimension_net_psi, 1).to(device)
        torch.nn.utils.vector_to_parameters(original_eta2, net_psi_0.parameters())

        for _ in range(u_iter):
            lossb = losses.PDHG_loss_X_bdd_norm_with_extraplt(
                net_u,
                net_phi,
                net_phi_0,
                net_psi,
                ...
                bd_lambda,
                omega,
            ) + bd_lambda * losses.bdryloss_X_norm_use_samples(net_u, bd_samples, bdd_idx)
            ...
            net_u_auxil = models.network_prim(network_length, config.dim, hidden_dimension_net_u, 1).to(device)
            net_u_auxil.load_state_dict(net_u.state_dict())

            G_inv_nabla_theta_loss, info_u = minres_utils.minres_solver_G(
                net_u,
                net_u_auxil,
                in_samples,
                bd_samples,
        ...
        computation_time.append(total_time)
        ...
        if (t + 1) % print_period == 0:
            print("Iter: {}, ".format(t))
            print("relative L2 error = {}".format(L2error / config.u_real_norm))
            print("H1 error = {}".format(H1error))
            print("boundary loss = {}".format(boundary_error))

    filename = os.path.join(save_path, "netu_NPDHG.pt")
    ...
    filename = os.path.join(save_path, "netpsi_NPDHG.pt")
    torch.save(net_psi.state_dict(), filename)

    with open("computation_time", "wb") as file_time:
        pickle.dump(computation_time, file_time)
    ...
    plotting_utils.plt.plot(computation_time, np.log(l2error_list / config.u_real_norm) / np.log(10), color="blue")
    ...
    fig_plot = plotting_utils.plt.figure(figsize=(15, 15))
    plotting_utils.plt.plot(computation_time, np.log(H1error_list / config.u_real_H1norm) / np.log(10), color="blue")
    plotting_utils.plt.xlabel("Iteration", fontsize=30)
